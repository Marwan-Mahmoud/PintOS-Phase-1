...
thread_yield(void)
{
  ...
  old_level = intr_disable();
  if (cur != idle_thread)
    list_insert_ordered(&ready_list, &cur->elem, higher_priority_comparator, NULL);
...
thread_set_priority(int new_priority)
{
  if (!thread_mlfqs) // Not advanced scheduler (i.e. priority scheduler).
  {
    ...
    {
      curr->priority = new_priority; // Thread has locks but the new_priority is higher than the donated priority, so change it to new_priority.
    }

    if (!list_empty(&ready_list))
    {
      list_sort(&ready_list, higher_priority_comparator, NULL);                            // Reorder ready list after changing priority.
      check_preemtion(list_entry(list_front(&ready_list), struct thread, elem)->priority); // Current thread yields if its priority is less than the highest priority of the ready list.
    }
  }
}
...
/* Updates thread's priority (advanced scheduler). */
void
update_priority()
{
...
// Updates recent CPU of running thread and ready threads (called every second).
void
update_recent_cpu_each(void)
{
  enum intr_level old_level = intr_disable();
...
{
  return real_to_int_rounded_to_nearest(mul_real_by_int(load_avg, 100));
}
